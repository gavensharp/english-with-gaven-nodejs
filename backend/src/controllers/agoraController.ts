...
export const generateRtcToken = async (req: Request, res: Response) => {
  try {
    ...
    const userId = uid ? parseInt(uid as string) : 0;
 ...
 */
export const generateRtmToken = async (req: Request, res: Response) => {
  try {
 ...
 */
export const renewRtcToken = async (req: Request, res: Response) => {
  // Reuse the same logic as generateRtcToken
  req.query = req.body;
  return generateRtcToken(req, res);
};
